...
impl<const XSIZE: usize, const YSIZE: usize, const CHARCNT: usize> Font<XSIZE, YSIZE, CHARCNT> {
    ...
    pub fn print_nth_music(self: &Self, chr: u8, bitmap: &mut Bitmap3, pos_x: usize, pos_y: usize) {
        let chr_ref = &(self.chars[chr as usize]);
    ...
    pub fn print_str_music(self: &Self, text: impl AsRef<str>, bitmap: &mut Bitmap3, pos_x: usize, pos_y: usize) { // Iterates a string and prints, with rudimentary line breaking!
        let mut pos_x = pos_x;
        let mut pos_y = pos_y;
        for chr in text.as_ref().chars() {
            if pos_x + XSIZE > 240 { // Break line automagically :3
    ...
    pub fn print_str_rel_music(self: &Self, text: impl AsRef<str>, bitmap: &mut Bitmap3, pos_x: usize, pos_y: usize) { // Prints a string but interprets the coords as a grid of tiles of XSIZE*YSIZE
        self.print_str_music(text, bitmap, pos_x * XSIZE, pos_y * YSIZE);
    }
}
